		      ...
		      unsigned int *len)
{
	...
	#ifdef XSD_DEBUG
		printf("[TOOL]xs_talkv, before write msg, type : %d\n",type);
	#endif
	msg.tx_id = t;
	msg.req_id = 0;
	msg.type = type;
	...
	{
 		#ifdef XSD_DEBUG
			printf("[TOOL]xs_write_all, fail to write header \n");
		#endif
		goto fail;
	}
	...
	for (i = 0; i < num_vecs; i++)
		...
		{   
			#ifdef XSD_DEBUG
				printf("[TOOL]xs_write_all, fail to write body\n");
			#endif
			goto fail;
			}
	...
	if (!ret)
		{
		#ifdef XSD_DEBUG
		printf("[TOOL]read reply  fail\n");
		#endif
		goto fail;
		}
	#ifdef XSD_DEBUG
        	printf("[TOOL]read reply  success\n");
	#endif
	pthread_mutex_unlock(&h->request_mutex);
	...
	if (msg.type == XS_ERROR) {
		...
		errno = saved_errno;
		#ifdef XSD_DEBUG
			printf("[TOOL]reply msg is error\n");
		#endif 
		return NULL;
	}
		       ...
		       unsigned int *len)
{
	struct iovec iovec;
	#ifdef XSD_DEBUG
      		 printf("[TOOL]%s,msg type : %d, body: %s\n",__FUNCTION__,type,string);
	 #endif 
	      ...
	      const char *path, const void *data, unsigned int len)
{
	...
	iovec[1].iov_len = len;
	#ifdef XSD_DEBUG
       printf("[TOOL] xs_write, data : %s , path : %s\n",(char*)data,path);
	#endif
			 ...
			 unsigned int eventchn)
{
	...
	iov[2].iov_len = strlen(eventchn_str) + 1;
		...
		for( i = 0; i <3 ; i++)
		{
			printf("%d th iov. base : %s, len : %d\n",i, (char*)iov[i].iov_base,iov[i].iov_len);
		}
       #endif
