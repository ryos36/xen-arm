...
class XendDomainInfo:
    ...
    def createChannels(self):
        ...
        """
        self.store_port = self.createChannel(use=ord('X')^ord('e')^ord('n')^ord('S')^ord('t')^ord('o')^ord('r')^ord('e'))
        self.console_port = self.createChannel(use=ord('v')^ord('c')^ord('o')^ord('n')^ord('s')^ord('o')^ord('l')^ord('e'))


    def createChannel(self, use):
        """Create an event channel to the domain.
        """
        try:
            return xc.evtchn_alloc_unbound(dom=self.domid, remote_dom=0, chn_use=use)
        except:
...
from xen.xend.server import blkif, netif, tpmif, pciif, iopif, irqif, usbif, mtdif, kppif, fbif, sif, bfsif
...
addControllerClass('vmtd',  mtdif.MtdifController)
addControllerClass('vkpp', kppif.KppifController)
addControllerClass('vfb', fbif.FbifController)
addControllerClass('vs', sif.SifController)
addControllerClass('vbfs', bfsif.BfsifController)
