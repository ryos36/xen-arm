...
/* flollowing definition has to shared with VMM */
#define CMD_GUEST_CREATE  0x01
#define CMD_GUEST_COPY  0x02

/* for guest domain image infomation */
#define DOM0_GUEST_IMAGE_CTL   60
typedef struct dom0_guest_image_info {
    unsigned long domain;                       /* domain id to load */
    unsigned long sub_cmd;                      /* domain id to load */
    unsigned long guest_image_address;          /* guest domain image physical address */
    unsigned long guest_image_size;             /* guest domain image size */
    unsigned int  store_mfn;                    /* to make sharing page between domain and xenstore -kcr */
    unsigned int  store_port;                   /* to make event channel between domain and xenstore -kcr */
    unsigned int  console_mfn;                  /* to make sharing page between console daemon and virtual serial driver - ldh */
    unsigned int  console_port;                 /* to make event channel between console daemon and virtual serial driver -ldh */
} dom0_guest_image_info_t;
DEFINE_XEN_GUEST_HANDLE(dom0_guest_image_info_t);
...
typedef struct dom0_op {
	...
	/* for domU image copy by PCJ */
        struct dom0_guest_image_info  guest_image_info;
